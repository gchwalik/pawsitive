...
function Container({ title, showTitleBorder = false, minHeight = true, className = "", children }: ContainerProps) {
  title = title ? title : "";
  return (
    <div className={`w-full sm:w-3/4 md:w-2/3 lg:w-1/2 bg-amber-50 rounded-lg m-3 pt-1 shadow-lg max-h-[70vh] ${minHeight ? `min-h-96 sm:min-h-[24rem] md:min-h-[32rem] lg:min-h-[40rem]` : ""} ${className}`}>
